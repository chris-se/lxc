...
int main(int argc, char *argv[])
{
	...
	if (my_args.argc) {
		attach_options.exec_function = lxc_attach_run_command;
		attach_options.exec_payload = (void*)my_args.argv;
	} else {
		attach_options.exec_function = lxc_attach_run_shell;
		attach_options.exec_payload = NULL;
	}

	ret = lxc_attach(my_args.name, my_args.lxcpath[0], &attach_options, &pid);
	if (ret < 0)
		return -1;
