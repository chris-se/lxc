...
int lxc_attach(const char* name, const char* lxcpath, lxc_attach_options_t* options, pid_t* attached_process)
{
	int ret, status;
	...
	init_pid = get_init_pid(name, lxcpath);
	if (init_pid < 0) {
	...
	if (options->namespaces == -1) {
		options->namespaces = lxc_get_clone_flags(name, lxcpath);
		/* call failed */
		if (options->namespaces == -1) {
...
int attach_child_main(void* data)
{
	...
	rexit(options->exec_function(options->exec_payload));
}

int lxc_attach_run_command(void* payload)
{
	char** args = (char**)payload;
